...
def import_with_retries(records,
                        ...
                        max_retries=3,
                        backoff_seconds=2):
    """Insert data using PostgreSQL with retry logic"""
    total = len(records)
    if total == 0:
    ...
    while idx < total:
        ...
        while attempt <= max_retries and not success:
            try:
                ...
                extras.execute_values(cur, insert_query, values, page_size=1000)
                conn.commit()
                ...
                if "timeout" in msg.lower() or "canceling statement" in msg.lower():
                    attempt += 1
